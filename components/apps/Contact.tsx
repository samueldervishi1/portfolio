  ...
  if (status === 'sent') {
     return (
        <div className="h-full flex flex-col items-center justify-center bg-white text-center p-8">
           ...
           <p className="text-gray-600 mt-2 text-xs">Your message has been successfully delivered to Samuel's inbox.</p>
           <button 
    ...
    <div className="h-full flex flex-col bg-[#ECE9D8] font-sans text-xs">
      ...
      <div className="bg-white p-2 flex flex-col gap-2 border-b border-[#9FBBE3]">
         ...
         <div className="flex items-center gap-2">
            ...
            <input className="flex-1 border border-[#7F9DB9] px-1 py-0.5 outline-none focus:border-[#245DDA]" placeholder="Your Email Address" required />
         </div>
         <div className="flex items-center gap-2">
            <label className="w-16 text-right text-gray-500">Subject:</label>
            <input className="flex-1 border border-[#7F9DB9] px-1 py-0.5 outline-none focus:border-[#245DDA]" placeholder="Project Inquiry..." />
         </div>
      </div>

      {/* Toolbar */}
      <div className="bg-[#ECE9D8] p-1 flex gap-1 border-b border-gray-300">
         <button className="flex items-center gap-1 px-2 py-1 border border-transparent hover:border-gray-400 hover:bg-white active:bg-gray-200" onClick={() => document.querySelector('form')?.requestSubmit()}>
            <Send size={14} />
            <span>Send</span>
         </button>
