...
const Taskbar: React.FC<TaskbarProps> = ({
  ...
  // Use external calendar state if provided, otherwise use internal state
  const calendarOpen = isCalendarOpen !== undefined ? isCalendarOpen : isCalendarPanelOpen;
  ...
  useEffect(() => {
    ...
    if (isWiFiPanelOpen || isVolumePanelOpen || calendarOpen || isNotificationPanelOpen) {
  ...
  }, [isWiFiPanelOpen, isVolumePanelOpen, calendarOpen, isNotificationPanelOpen]);

  const closeAllPanels = () => {
    setIsWiFiPanelOpen(false);
    setIsVolumePanelOpen(false);
    if (onCalendarToggle && calendarOpen) {
      onCalendarToggle();
    } else {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-10 bg-win-taskbar flex items-center z-[100] glass border-t border-white/10">
      ...
      >
        ...
        >
          <path
            d="M0 3.42857L9.42857 2.14286V11.5714H0V3.42857Z"
            fill={isStartOpen ? "#0078D7" : "white"}
            className="group-hover:fill-[#0078D7] transition-colors"
          />
          <path
            d="M10.2857 2L24 0V11.5714H10.2857V2Z"
            fill={isStartOpen ? "#0078D7" : "white"}
            className="group-hover:fill-[#0078D7] transition-colors"
          />
          <path
            d="M0 12.4286H9.42857V20.5714L0 19.2857V12.4286Z"
            fill={isStartOpen ? "#0078D7" : "white"}
            className="group-hover:fill-[#0078D7] transition-colors"
          />
          <path
            d="M10.2857 12.4286H24V24L10.2857 22V12.4286Z"
            fill={isStartOpen ? "#0078D7" : "white"}
            className="group-hover:fill-[#0078D7] transition-colors"
          />
        </svg>
        ...
        {windows
          ...
          .map((window) => (
            ...
            >
              <window.icon
                size={16}
                className={activeWindowId === window.id ? "text-blue-400" : "text-gray-400"}
              />
      ...
      <div className="h-full flex items-center px-2 gap-1 relative">
        ...
        <button
          ...
          className={`h-full px-2 hover:bg-white/10 flex flex-col items-end justify-center cursor-pointer text-white min-w-[70px] rounded-sm transition-colors ${
            calendarOpen ? "bg-white/10" : ""
          }`}
        >
          <span className="text-xs leading-none mb-[2px]">
        ...
        <CalendarPanel isOpen={calendarOpen} onClose={closeAllPanels} />
