...
import { User, Power, Settings, FileText, Folder, FileType, Github, Linkedin, Mail, Calendar } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="w-64 p-4 overflow-y-auto hidden sm:block scrollbar-hide">
        ...
        <button className="w-full text-left px-2 py-2 hover:bg-white/10 flex items-center gap-3 rounded" onClick={() => onItemClick('about')}>
           <FileText size={16} className="text-blue-400" />
           <span className="text-sm">About Me</span>
        </button>

        <div className="text-xs font-semibold text-gray-400 mb-2 mt-2 pl-2">C</div>
        <button className="w-full text-left px-2 py-2 hover:bg-white/10 flex items-center gap-3 rounded" onClick={() => window.open('mailto:samuel@example.com')}>
           <Mail size={16} className="text-blue-400" />
           <span className="text-sm">Contact</span>
        </button>

        <div className="text-xs font-semibold text-gray-400 mb-2 mt-2 pl-2">P</div>
        <button className="w-full text-left px-2 py-2 hover:bg-white/10 flex items-center gap-3 rounded" onClick={() => onItemClick('projects')}>
           <Folder size={16} className="text-yellow-400" />
           <span className="text-sm">Projects</span>
        </button>

        <div className="text-xs font-semibold text-gray-400 mb-2 mt-2 pl-2">R</div>
        <button className="w-full text-left px-2 py-2 hover:bg-white/10 flex items-center gap-3 rounded" onClick={() => onItemClick('resume')}>
